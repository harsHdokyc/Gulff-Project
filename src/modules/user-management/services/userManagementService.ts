...
export interface ProBusinessRequest {
  ...
  created_at: string;
  updated_at: string;
  // Business name populated by service
...
export class UserManagementService {
  ...
  async getUsers(params?: { search?: string; role?: string }): Promise<{ success: boolean; data?: User[]; total?: number; error?: string }> {
    try {
...
// Pagination (disabled for user management — small fixed-size lists)
      ...
      // Apply ordering (user management returns full filtered set)
  ...
  // Update user
  async updateUser(userId: string, updates: Partial<Pick<User, 'display_name'>>): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      // For now, we'll skip updating since display_name is primarily stored in auth metadata
      // The display name will be fetched from auth metadata in the component
  ...
  async createProAssociationRequest(request: CreateProAssociationRequest): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      // Get business name first (owner has access to their own company)
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          business_name: businessName, // This will work after migration
        });
  ...
  async getBusinessAssociationRequests(): Promise<{ success: boolean; data?: ProBusinessRequest[]; error?: string }> {
    try {
      ...
      // Fetch PRO user data separately to avoid RLS issues
      const requests = await Promise.all((data || []).map(async (item: any) => {
        let proUserData = null;
        if (item.pro_user_id) {
          const { data: proUser } = await supabase.rpc('get_user_by_id', { 
              user_id: item.pro_user_id 
            }).maybeSingle();
          proUserData = proUser;
        }
        ...
        return {
          ...
          pro_name: proUserData?.full_name || 'PRO User',
          pro_email: proUserData?.email || `PRO User (${item.pro_user_id?.slice(0, 8)}...)`,
        };
      }));
  ...
  async getProAssociationRequests(): Promise<{ success: boolean; data?: ProBusinessRequest[]; error?: string }> {
    ...
    try {
      ...
      // Get requests with business_name directly from the table
      const { data, error } = await supabase
        .from('pro_business_requests')
        .select(`
          id,
      ...
      // Process requests - business_name is now directly from the table
      const requestsWithNames = data.map((item: any) => ({
        id: item.id,
        business_id: item.business_id,
        pro_user_id: item.pro_user_id,
        status: item.status,
      ...
      return { success: true, data: requestsWithNames };
    } catch (error) {
      return { success: false, error: error.message };
    }
  }
